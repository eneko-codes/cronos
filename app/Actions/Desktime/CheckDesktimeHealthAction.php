...
/**
 * Action to check the health of the DeskTime API and notify maintenance users on failure.
 *
 * This action is typically called from sync job failure handlers when DeskTime
 * sync jobs fail. It performs a health check ping to the DeskTime API and,
 ...
 * @see \App\Jobs\Sync\Desktime\SyncDesktimeUsersJob Typical caller
 * @see \App\Jobs\Sync\Desktime\SyncDesktimeAttendancesJob Typical caller
 */
class CheckDesktimeHealthAction
{
    /**
     * Execute the DeskTime API health check and send notifications if the API is down.
     *
     * This method:
     * 1. Performs a ping health check to the DeskTime API
     ...
     *
     * @param  DesktimeApiClient  $client  The DeskTime API client instance
    ...
    public function __invoke(DesktimeApiClient $client): array
    {
        ...
        if (! ($result['success'] ?? false)) {
            ...
            foreach ($maintenanceUsers as $maintenanceUser) {
                ...
                if ($isEligible && ApiDownWarning::shouldSend($maintenanceUser, 'DeskTime')) {
