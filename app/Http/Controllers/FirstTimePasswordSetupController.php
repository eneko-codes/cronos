...
class FirstTimePasswordSetupController extends Controller
{
     ...
     *
     * @param  Request  $request  The HTTP request containing token and email from URL
     * @return View The password setup form view with token validation status
     *
     * @throws HttpException 403 if token or email is missing
     * @throws NotFoundHttpException 404 if user not found or already has password
     */
    public function create(Request $request): View
    {
        ...
        // Prevent archived users from setting up password
        if (! $user->is_active) {
            abort(403, 'Your account has been archived and you cannot set up a password.');
        }
     ...
     * best practices.
     *
     * @param  FirstTimePasswordSetupRequest  $request  The validated password setup request containing email, token, and new password
     * @return RedirectResponse Redirects to dashboard on success, back with errors on failure
     *
    ...
    public function store(FirstTimePasswordSetupRequest $request): RedirectResponse
    {
        ...
        // Check if user is archived before attempting password setup
        $user = User::where('email', $validatedData['email'])->whereNull('password')->first();
        if ($user && ! $user->is_active) {
            return back()->withErrors(['email' => 'Your account has been archived and you cannot set up a password.']);
        }
