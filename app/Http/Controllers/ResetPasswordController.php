...
class ResetPasswordController extends Controller
{
     ...
     * @return View The password reset form view with token validation status
     */
     ...
     * best practices.
     *
     * @param  ResetPasswordRequest  $request  The validated password reset request containing email, token, and new password
     * @return RedirectResponse Redirects to login page on success, back with errors on failure
     *
    ...
    public function store(ResetPasswordRequest $request): RedirectResponse
    {
        ...
        // Check if user is archived before attempting reset
        $user = User::where('email', $validatedData['email'])->first();
        if ($user && ! $user->is_active) {
            return back()->withErrors(['email' => 'Your account has been archived and you cannot reset your password.']);
        }
