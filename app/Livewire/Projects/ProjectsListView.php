        ...
        $applyFiltersAndSorting = function ($query): void {
            ...
            $query->when($this->filters['has_tasks'], function ($q): void {
                ...
                ->when($this->filters['has_direct_time_entries'], function ($q): void {
                    $q->whereHas('timeEntries', function (Builder $subQuery): void {
                        /** @var Builder<TimeEntry> $subQuery */
                        $subQuery->projectLevel();
                    });
                })
                ->withCount('tasks')
                ->withCount([
                    'timeEntries as project_time_entries_count' => function (Builder $subQuery): void {
                        /** @var Builder<TimeEntry> $subQuery */
                        $subQuery->projectLevel();
                    },
                ])
