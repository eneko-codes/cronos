...
class GlobalNotificationSettings extends Component
{
    ...
    public function mount(NotificationService $notificationService): void
    {
        ...
        // Authorize: only admins and maintenance users can view global settings
        $this->authorize('viewGlobalSettings');

        $settings = $notificationService->getPreferences($user);
    ...
    public function groupedNotificationTypes(): array
    {
        ...
        $typesByGroup = collect(NotificationType::cases())
            ->filter(fn (NotificationType $type) => ! $type->isMandatory())
    ...
    public function updatedGlobalNotificationsEnabled(
        ...
        // Authorize: only admins and maintenance users can manage global settings
        $this->authorize('manageGlobalSettings');

        try {
            $notificationService->toggleGlobalMaster($user, $value);
    ...
    public function updatedNotificationChannel(
        ...
        try {
            ...
            $notificationService->updateChannel($user, $value);
    ...
    public function updatedNotificationStates(
        ...
        try {
            ...
            $notificationService->toggleGlobalType($user, $notificationType, $value);
