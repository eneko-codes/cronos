...
class UserDashboard extends Component
{
  ...
  /**
   * Helper method to calculate styling and content data for a deviation table cell.
   *
   * @param array|null $detail The deviation detail array (containing percentage, tooltip, etc.) or null.
   * @param bool $isFutureDate Whether the date is in the future.
   * @return array An array containing keys: 'should_display', 'bg_class', 'text_class', 'percentage_text', 'tooltip_text'.
   */
  public function getDeviationCellData(
    ?array $detail,
    bool $isFutureDate
  ): array {
    $percentage = $detail ? $detail['percentage'] : 0;
    $tooltip = $detail ? $detail['tooltip'] : '';
    // Determine if the cell should display content (not future, has details, percentage is not zero)
    $shouldDisplay = !$isFutureDate && $detail && $percentage !== 0;
    ...
    if ($shouldDisplay) {
      // Determine background and text classes based on the percentage value
      if ($percentage > 0) {
        $bgClass = 'bg-green-100 dark:bg-green-900/30';
        $textClass = 'text-green-600 dark:text-green-600';
      } elseif ($percentage <= -50) {
        $bgClass = 'bg-red-100 dark:bg-red-900/30';
        $textClass = 'text-red-600 dark:text-red-600';
      } elseif ($percentage < 0) {
        $bgClass = 'bg-yellow-100 dark:bg-yellow-900/30';
        $textClass = 'text-yellow-500 dark:text-yellow-500';
      }
      // Format the percentage text (e.g., +10%, -5%, 20%)
      $percentageText = ($percentage > 0 ? '+' : '') . $percentage . '%';
    }

    // Return all necessary data for the Blade view to render the cell
    return [
      'should_display' => $shouldDisplay,
      'bg_class' => $bgClass,
      'text_class' => $textClass,
      'percentage_text' => $percentageText,
      'tooltip_text' => $tooltip, // Tooltip text is always needed by the tooltip component
    ];
  }
}
