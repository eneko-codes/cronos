...
class UserAdminActions extends Component
{
    ...
    public function loadUserDetails(): void
    {
        ...
        $this->canDemoteAdmin = $user->isAdmin() && Gate::allows('demoteAdmin', $user);
        $this->canPromoteToMaintenance = Gate::allows('promoteToMaintenance', $user);
        $this->canDemoteFromMaintenance = $user->isMaintenance() && Gate::allows('demoteFromMaintenance', $user);
        $this->canNotTrack = ! $user->do_not_track && Gate::allows('disableTracking', $user);
        $this->canEnableTracking = $user->do_not_track && Gate::allows('enableTracking', $user);
        $this->canArchiveUser = $user->is_active && Gate::allows('archiveUser', $user);
    }
