 ...
 * @property Carbon $date Date when the work was performed (stored in UTC timezone)
 * @property int $duration_seconds
 * @property Carbon $proofhub_created_at When the entry was created in ProofHub
 * @property Carbon|null $created_at When record was created locally
 * @property Carbon|null $updated_at When record was last updated locally
 * @property-read CarbonInterval $duration The duration as a Carbon interval
 * @property-read string $formatted_duration The formatted duration string (e.g., "2h 30m")
 * @property-read User $user The user who logged this time
 * @property-read Project $project The project this time entry belongs to
 * @property-read Task|null $task The task this time entry is associated with (if any)
 *
...
class TimeEntry extends Model
{
    ...
    {
        return Attribute::make(
            get: fn () => CarbonInterval::seconds((int) $this->duration_seconds)
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => DurationFormatterService::fromSeconds((int) $this->duration_seconds)
        );
    }
     ...
     */
    #[Scope]
    protected function betweenDates(Builder $query, CarbonInterface|string $from, CarbonInterface|string $to): void
    {
    ...
    /**
     * Scope a query to only include project-level time entries (not associated with a task).
     *
     * These are time entries logged directly against a project, not a specific task.
     */
    #[Scope]
    protected function projectLevel(Builder $query): void
    {
        $query->whereNull('proofhub_task_id');
    }

    /**
     * Scope a query to only include time entries for a specific date.
     */
    #[Scope]
