 ...
 * @property Carbon $date Date of the attendance record (stored in UTC)
 ...
 * @property Carbon|null $clock_in Start time of attendance (if available)
 * @property Carbon|null $clock_out End time of attendance (if available)
 ...
 * @property-read User $user The user this attendance record belongs to
 *
...
class UserAttendance extends Model
{
    ...
    {
        return Attribute::make(
            get: fn () => CarbonInterval::seconds((int) ($this->duration_seconds ?? 0))
        );
    }
    ...
    {
        return Attribute::make(
            get: fn () => DurationFormatterService::fromSeconds((int) ($this->duration_seconds ?? 0))
        );
    }
    ...
    /**
     * Scope a query to only include remote work attendance records (from DeskTime).
     */
    #[Scope]
    protected function remote(Builder $query): void
    {
        $query->where('is_remote', true);
    }

    /**
     * Scope a query to only include on-site attendance records (from SystemPin).
     */
    #[Scope]
