...
class DatabaseSeeder extends Seeder
{
  ...
  public function run(): void
  {
    ...
    echo "📅 Generating data for time period: {$from->format(
      'Y-m-d'
    )} to {$to->format('Y-m-d')}\n\n";
    ...
    echo '✅ Full-time schedules assigned to ' .
      count($fullTimeUsers) .
      " users!\n\n";
    ...
    echo '✅ Part-time schedules assigned to ' .
      count($partTimeUsers) .
      " users!\n\n";
  ...
  private function resetAutoIncrement(): void
  {
    ...
    echo "    • Dropping all data from database tables...\n";
    foreach ($tables as $table) {
      // For SQLite, first delete all data
      DB::table($table)->truncate();

      // Then reset the SQLite auto-increment counters
    ...
    foreach ($tables as $table) {
      ...
      if ($count > 0) {
        echo "      ! Warning: Table '{$table}' still has {$count} records\n";
        // Try harder to remove data
  ...
  private function assignTasksToUser(User $user): void
  {
    ...
    if ($user->name === 'John Developer') {
      ...
      $designerTasks = [];
      if ($frontendTasks->count() >= 1) {
        $designerTasks[] = $frontendTasks[0]->proofhub_task_id;
      }
      if ($frontendTasks->count() >= 2) {
        $designerTasks[] = $frontendTasks[1]->proofhub_task_id;
      }
      if ($frontendTasks->count() >= 4) {
        $designerTasks[] = $frontendTasks[3]->proofhub_task_id;
      }
      ...
      $engineerTasks = [];
      if ($apiTasks->count() >= 2) {
        $engineerTasks[] = $apiTasks[1]->proofhub_task_id;
      }
      if ($apiTasks->count() >= 4) {
        $engineerTasks[] = $apiTasks[3]->proofhub_task_id;
      }
      if ($mobileTasks->count() >= 1) {
        $engineerTasks[] = $mobileTasks[0]->proofhub_task_id;
      }
      if ($mobileTasks->count() >= 2) {
        $engineerTasks[] = $mobileTasks[1]->proofhub_task_id;
      }
      ...
      $testerTasks = [];
      if ($apiTasks->count() >= 1) {
        $testerTasks[] = $apiTasks[0]->proofhub_task_id;
      }
      if ($apiTasks->count() >= 3) {
        $testerTasks[] = $apiTasks[2]->proofhub_task_id;
      }
      if ($frontendTasks->count() >= 3) {
        $testerTasks[] = $frontendTasks[2]->proofhub_task_id;
      }
      if ($frontendTasks->count() >= 4) {
        $testerTasks[] = $frontendTasks[3]->proofhub_task_id;
      }
      ...
      $managerTasks = [];
      if ($apiTasks->count() >= 3) {
        $managerTasks[] = $apiTasks[2]->proofhub_task_id;
      }
      if ($frontendTasks->count() >= 5) {
        $managerTasks[] = $frontendTasks[4]->proofhub_task_id;
      }
      if ($mobileTasks->count() >= 5) {
        $managerTasks[] = $mobileTasks[4]->proofhub_task_id;
      }
  ...
  private function createTimeData(array $users, float $scheduleHours): void
  {
    ...
    echo '  🕒 Creating attendance and time entry data for ' .
      count($users) .
      ' ' .
      $scheduleType .
      " users...\n";
      ...
      if (!$isWeekend || $isMonday || rand(1, 100) <= 15) {
        foreach ($users as $user) {
          ...
          if (!$hasLeave) {
            ...
            $newTimeEntries = $this->createTimeEntriesForUser(
              $user,
              $currentDate,
              $timeEntryHours
            );
  ...
  private function createTimeEntriesForUser($user, $date, $totalHours): int
  {
    ...
    $totalSeconds = intval(round($totalHours * 3600));
    $usedSeconds = 0;

    foreach ($userProjects as $project) {
      // Find tasks for this project
      $projectTasks = $this->tasks->where(
        'proofhub_project_id',
        $project->proofhub_project_id
      );
      ...
      foreach ($selectedTasks as $task) {
        ...
        $entrySeconds = intval(round($entryHours * 3600));

        // Generate a unique ID for the time entry - ensure uniqueness
        $uniqueId = $this->generateUniqueTimeEntryId();
   ...
   */
  private function createGuaranteedLeaves(
    $users,
    Carbon $from,
    Carbon $to
  ): void {
  ...
  /**
   * Generate a unique time entry ID that doesn't exist in the database
   */
  private function generateUniqueTimeEntryId(): int
  {
    do {
