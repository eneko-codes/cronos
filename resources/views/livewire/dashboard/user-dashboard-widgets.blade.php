<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
  <!-- Today's Schedule -->
  <div
    class="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800"
  >
    <div class="flex items-center gap-2">
      ...
      <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">
        @if ($todaysLeave && ! $todaysLeave['isHalfDay'])
          Today's Leave
        @else
            Today's Schedule
        @endif
    ...
    @if ($todaysSchedule)
      <div class="flex flex-col">
        @if ($todaysLeave && ! $todaysLeave['isHalfDay'])
          {{-- Full day leave - show leave as primary info --}}
          <div class="flex items-center justify-between">
            <span class="text-lg font-bold text-amber-700 dark:text-amber-400">
              {{ $todaysLeave['type'] }}
            </span>
            @if ($todaysLeave['status'] === 'confirm')
              <x-badge variant="warning" size="sm">Pending</x-badge>
            @endif
          </div>

          <span class="mb-2 text-xs text-gray-500 dark:text-gray-400">
            {{ $todaysLeave['timePeriod'] }}
          </span>

          {{-- Show minimal schedule info --}}
          <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            Scheduled: {{ $todaysSchedule['duration'] }}
            ({{ $todaysSchedule['name'] }})
          </div>
        @elseif ($todaysSchedule['duration'] && $todaysSchedule['duration'] !== '0h 0m')
          ...
          {{-- Today's Leave Information (only for half-day leaves) --}}
          @if ($todaysLeave && $todaysLeave['isHalfDay'])
            <div
            ...
            >
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400">
                {{ $todaysLeave['timePeriod'] }}
                @if ($todaysLeave['timeRange'])
                  <span class="text-gray-600 dark:text-gray-300">
                    ({{ $todaysLeave['timeRange'] }})
                  </span>
                @endif
              </div>
            </div>
        ...
        @else
          ...
          {{-- Today's Leave Information for Day Off --}}
          @if ($todaysLeave)
            <div
              class="mt-3 border-t border-gray-200 pt-3 dark:border-gray-600"
            >
    ...
    @else
      ...
      {{-- Today's Leave Information when no schedule --}}
      @if ($todaysLeave)
