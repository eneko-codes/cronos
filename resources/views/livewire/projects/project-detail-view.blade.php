      ...
      @if ($showTasks)
        <div class="flex flex-col gap-2">
          @forelse ($tasks as $task)
            @php($entryCount = $taskTimeEntryCounts[$task->proofhub_task_id] ?? 0)
            <!-- Task Container -->
            <div
            ...
            >
              <!-- Task Header Row -->
              <div
                class="{{ $entryCount > 0 ? 'cursor-pointer rounded-t-md' : 'rounded-md' }} flex flex-row items-center justify-between gap-2 p-2 hover:bg-gray-50 dark:hover:bg-gray-600/50"
                @if ($entryCount > 0) wire:click="toggleTask('{{ $task->proofhub_task_id }}')" @endif
              >
                <!-- Task Info -->
