...
describe('NotificationService notification dispatching', function (): void {
    it('sends notifications to maintenance users with eager loaded preferences', function (): void {
        ...
        $service = app(NotificationService::class);
        $notification = new ApiDownNotification('Test Service', 'Test error message');

        Notification::fake();

        $service->notifyMaintenanceUsers($notification);

        Notification::assertSentTo([$maintenance1, $maintenance2], ApiDownNotification::class);
        Notification::assertNotSentTo(
            User::where('email', 'inactive@test.com')->first(),
            ApiDownNotification::class
        );
    });
    ...
    it('sends notifications to admin users excluding specified user', function (): void {
        ...
        $service = app(NotificationService::class);
        $notification = new AdminPromotionNotification($admin1, $admin1);
        ...
        Notification::assertNotSentTo($admin1, AdminPromotionNotification::class);
        Notification::assertSentTo($admin2, AdminPromotionNotification::class);
    });
    ...
    it('sends notification to single user', function (): void {
        ...
        $service = app(NotificationService::class);
        $notification = new ScheduleChangeNotification($user, null, null);

        Notification::fake();

        $service->notifyUser($user, $notification);

        Notification::assertSentTo($user, ScheduleChangeNotification::class);
    });
});
